...
describe('ContractService', () => {
  ...
  const makeDeps = () => {
    ...
    const historyService = {
      ...
      findSpendsBySource: jest.fn(),
      findIncomesBySource: jest.fn(),
      createSpendsWithSource: jest.fn().mockResolvedValue(undefined),
      createIncomesWithSource: jest.fn().mockResolvedValue(undefined),
    };

    const blockService = {
      getCachedBlock: jest.fn(),
      findBlockByTimestamp: jest.fn(),
      getBlocksPerDay: jest.fn().mockReturnValue(43_200),
  ...
  describe('saveReserves', () => {
    ...
    it('uses source.startBlock when provided startDate is older than start block timestamp', async () => {
      ...
      expect(blockService.getCachedBlock).toHaveBeenNthCalledWith(
        1,
        'eth',
        provider,
        source.startBlock,
    ...
    it('caps start block to source.endBlock when startDate resolves to a higher block', async () => {
      ...
      blockService.findBlockByTimestamp.mockResolvedValue(9_999);
    ...
    it('stops daily processing after reaching endBlock and does not write duplicate future-day rows', async () => {
      ...
      expect(blockService.findBlockByTimestamp).toHaveBeenCalledTimes(2);
